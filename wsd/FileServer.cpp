                                                     ...
                                                     const std::shared_ptr<StreamSocket>& socket)
{
    try
    {
        ...
        /*
            So now we want to do below things

            - first extract token from we sent headers: {
                        Authorization ... } 
                        which can be used to authenticate

            - then we have to create one wopi URL (May be we have to create new endpoint wopi endpoint called `settings/upload` )
            - then send file to that urls... then we can go to nextcloud and will handle that end point.
