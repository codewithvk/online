...
class RotateAndScaleDepthByWidth extends Operation {
	...
	public interpolate(matrix : mat4, t: number, SlideWidthScale: number, SlideHeightScale: number): mat4 {
        if (t <= this.mnT0) {
        ...
        const translationVector = vec3.fromValues(SlideWidthScale * this.origin[0], SlideHeightScale * this.origin[1], SlideWidthScale * this.origin[2]);
        const scaleVector = vec3.fromValues(SlideWidthScale * SlideWidthScale, SlideHeightScale * SlideHeightScale, 1);
